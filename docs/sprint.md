...
## Sprint 1 — Scaffold and a running dev server [NOT STARTED]
...
- [ ] `tests/e2e/**` is excluded from Vitest collection — a committed placeholder spec under `tests/e2e/` does not appear in `npm run test`'s collected-file list, asserted from `--reporter=json` output.
...
- [ ] A grep test asserts zero occurrences of `src/dev`, `stubEngine` or `dev/fixtures` across `src/` and `tests/`, excluding `tests/lint/fixtures/`.
...
|---|---|---|---|
| Sprint 1 trial (PR #3) | `tests/scaffold/tree.test.ts` asserts a tracked file under `src/dev/`; Sprint 46 deletes that directory, reddening a test in `tests/scaffold/**`, which no sprint after 1 owns. Either give Sprint 46 that path or let the assertion self-retire when the directory is legitimately gone. | Sprint 46 | OPEN |
